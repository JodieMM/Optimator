...
namespace Animator
{
    ...
    public class Scene
    {
        ...
        public void AssignOriginalPositions()
        {
            ...
            {
                Piece holdPiece;
                if (partOrder[index].StartsWith("p"))
                {
                    holdPiece = piecesList[int.Parse(partOrder[index].Remove(0, 2))];
                }
                else
                {
                    holdPiece = setList[int.Parse(partOrder[index].Remove(0, 2))].GetBasePiece();
                }
